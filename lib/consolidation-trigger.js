...
import { OM_OBSERVATIONS_DROPPED, OM_OBSERVATIONS_RECORDED, OM_REFLECTIONS_RECORDED, buildObservationsDroppedData, buildObservationsRecordedData, buildReflectionsRecordedData, appendRecord, foldLedger, fullProjection, latestCoverageSeq, loadLedger, observationToSummaryLine, rawTokensSinceSurfaceAnchor, realTokensSinceAnchor, reflectionToSummaryLine, } from "./ledger/index.js";
 ...
 */
export function surfaceEventsOf(agent) {
 ...
 */
function sourceEventsAfter(events, afterSeq) {
    return events.filter((event) => (event.type === "user/message" || event.type === "assistant/message" || event.type === "tool/result")
        && (afterSeq === undefined || event.seq > afterSeq));
}
function runtimeCtxOf(ctx) {
    return {
        hasLogger: true,
        logger: {
...
export function meterTokens(ctx, agent) {
    try {
 ...
 * over-fire or starve.
 */
export function stageProgress(ledger, currentTokens, coverageType, surfaceEvents) {
...
function anyStageDue(ledger, runtime, currentTokens, surfaceEvents) {
...
export function registerConsolidationTrigger(ctx, runtime) {
    const maybeLaunch = (agent) => {
...
function maybeLaunchConsolidation(ctx, runtime, agent) {
    ...
    if (!anyStageDue(ledger, runtime, currentTokens, surfaceEvents))
        return;
...
export async function runConsolidationPipeline(ctx, runtime, agent, cwd, ledger) {
    const runtimeCtx = runtimeCtxOf(ctx);
    const resolveTargetOnce = () => {
    ...
    try {
        ...
        if (reflectorResult.outcome === "abort")
            return;
    }
    catch (error) {
...
async function runObserverStage(ctx, runtime, agent, cwd, ledger, resolveTargetOnce) {
    const surfaceEvents = surfaceEventsOf(agent);
    const currentTokens = meterTokens(ctx, agent);
    ...
    if (backoff) {
        if (backoff.sessionIdentity !== sessionId
    ...
    const { text: chunk, sourceEventSeqs, estimatedTokens: chunkTokens, truncatedSourceEventSeqs, } = serializeSourceAddressedEvents(backlogEvents, { maxTokens: maxChunkTokens });
    if (!chunk.trim() || sourceEventSeqs.length === 0)
        return "continue";
    ...
    if (currentTokens !== undefined)
        ledger.anchors.observations = { atSeq: agent.session.seq, tokens: currentTokens };
...
async function runReflectorStage(ctx, runtime, agent, cwd, ledger, resolveTargetOnce) {
    const surfaceEvents = surfaceEventsOf(agent);
    const currentTokens = meterTokens(ctx, agent);
    ...
    if (observationCoverageSeq === undefined)
        return { outcome: "continue", sameRunReflections: [] };
    const runtimeCtx = runtimeCtxOf(ctx);
    ...
    if (!data)
        return { outcome: "continue", sameRunReflections: [] };
    if (currentTokens !== undefined)
        ledger.anchors.reflections = { atSeq: agent.session.seq, tokens: currentTokens };
...
async function runDropperStage(ctx, runtime, agent, cwd, ledger, resolveTargetOnce, sameRunReflections, sameRunReflectionCoverageSeq) {
    ...
    const runtimeCtx = runtimeCtxOf(ctx);
    runtime.notify(runtimeCtx, `Observational memory: dropper running after reflection — active observation pool ~${metrics.observationTokens.toLocaleString()} / ${metrics.targetTokens.toLocaleString()} target tokens (${Math.round(metrics.fullness * 100).toLocaleString()}%)`, "info");
    const target = resolveTargetOnce();
    if (!target)
        return "abort";
    ...
    if (data)
        appendRecord(cwd, ledger, { type: OM_OBSERVATIONS_DROPPED, data });
    return "continue";
}
...
//# sourceMappingURL=consolidation-trigger.js.map