import { OM_COMPACTION, OM_OBSERVATIONS_DROPPED, OM_OBSERVATIONS_RECORDED, OM_REFLECTIONS_RECORDED, isCompactionData, isObservationsDroppedData, isObservationsRecordedData, isReflectionsRecordedData, } from "./types.js";
...
function foldProjection(records, boundaries) {
    const observations = [];
    const reflections = [];
    const observationsById = new Set();
    const reflectionsById = new Set();
    const droppedObservationIds = new Set();
    // An undefined boundary is the tip: every record qualifies (pi's tipBoundary).
    const covered = (record, boundary) => boundary === undefined || (coverageSeq(record) >= 0 && coverageSeq(record) <= boundary);
...
export function fullProjection(records, throughSeq) {
 ...
 */
export function visibleProjection(records) {
    for (let i = records.length - 1; i >= 0; i--) {
        const record = records[i];
        if (record?.type !== OM_COMPACTION || !isCompactionData(record.data))
            continue;
 ...
 */
export function buildCompactionProjection(records, firstKeptSeq, config) {
    ...
    const observationTokens = normalProjection.observations.reduce((total, observation) => total + observation.tokenCount, 0);
...
export function diffProjection(visible, full) {
