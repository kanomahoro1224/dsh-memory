...
import { OM_COMPACTION, appendRecord, buildCompactionProjection, loadLedger, renderSummary, } from "../ledger/index.js";
...
function basicConfigFrom(config) {
    ...
    if (om.retainTokens !== undefined)
        basic.retainTokens = om.retainTokens;
    else if (om.retainRatio !== undefined)
        basic.retainRatio = om.retainRatio;
    if (om.summarizationMaxTokens !== undefined)
        basic.maxTokens = om.summarizationMaxTokens;
    if (om.model) {
    ...
    if (Array.isArray(config.modelPolicies))
        basic.modelPolicies = config.modelPolicies;
    if (typeof config.compactionRetries === "number")
        basic.compactionRetries = config.compactionRetries;
    if (typeof config.maxOverflowRetries === "number")
        basic.maxOverflowRetries = config.maxOverflowRetries;
    return basic;
}
/**
 ...
 */
export function resolveFirstKeptSeq(session, messages) {
    ...
    for (const seq of surfaceNodes) {
        ...
        if (!event || event.type === "system/message")
            continue;
        const message = deriveEventMessage(event);
        if (message)
            idToSeq.set(message.id, seq);
    }
    for (let i = messages.length - 1; i >= 0; i--) {
        ...
        if (idx >= 0 && idx + 1 < surfaceNodes.length)
            return surfaceNodes[idx + 1];
        return session.seq;
    }
    return session.seq;
}
function meterTokens(ctx, agent) {
    try {
...
export class ObservationalMemoryEngine extends BasicCompactionEngine {
     ...
     */
    recordCompaction(agent, result, preNodes) {
        try {
            const cwd = agent.session.header.cwd ?? process.cwd();
            const ledger = loadLedger(cwd, agent.session.id);
            ...
            if (tokens !== undefined)
                ledger.anchors.compaction = { atSeq: agent.session.seq, tokens };
