...
export function normalizeSourceEventSeqs(
	...
	for (const value of sourceEventSeqs) {
		if (typeof value !== "number" || !Number.isSafeInteger(value) || !allowedOrder.has(value)) return undefined;
		seen.add(value);
	}
...
export async function runObserver(args: RunObserverArgs): Promise<Observation[] | undefined> {
	...
	const recordObservations = {
		...
		execute: (raw: unknown) => {
			const params = (raw ?? {}) as {
				observations?: Array<{ timestamp?: unknown; content?: unknown; relevance?: unknown; sourceEventSeqs?: readonly unknown[] }>;
			};
			let added = 0;
			let duplicates = 0;
			let rejected = 0;
			for (const proposal of params.observations ?? []) {
				...
				if (!sourceEventSeqs || typeof proposal.content !== "string" || proposal.content.length === 0
					...
					|| typeof proposal.relevance !== "string" || !(RELEVANCE_VALUES as readonly string[]).includes(proposal.relevance)) {
					rejected++;
					continue;
				}
