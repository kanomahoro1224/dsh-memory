...
export function summarizeCoverageTransitionsByRelevance(
	...
	const summary = {
		low: {} as Record<string, { count: number; tokens: number }>,
		medium: {} as Record<string, { count: number; tokens: number }>,
		high: {} as Record<string, { count: number; tokens: number }>,
		critical: {} as Record<string, { count: number; tokens: number }>,
	};
	for (const observation of observations) {
