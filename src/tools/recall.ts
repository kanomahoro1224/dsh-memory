...
/**
 * Canonical JSON value returned by the recall tool. `render()` projects the
 * `text` field to the model-facing content.
 */
type RecallToolValue = {
	status: string;
	memoryId: string;
	collision: boolean;
	partial: boolean;
	...
	/** Model-facing rendering of the recalled evidence. */
	text: string;
};

function observationLine(observation: RecalledObservation["observation"], status: "active" | "dropped"): string {
...
function renderObservationOnlyText(result: Extract<RecallResult, { status: "found" }>): string {
	const sections: string[] = [];
	if (result.collision) sections.push(`Memory id ${result.memoryId} matched multiple observations; returning all matching source results.`);
...
function renderMemoryText(result: Extract<RecallResult, { status: "found" }>): string {
	const sections: string[] = [];
	if (result.collision) sections.push(`Memory id ${result.memoryId} matched multiple observations/reflections; returning all available evidence.`);
...
function foundValue(result: Extract<RecallResult, { status: "found" }>): RecallToolValue {
	return {
...
export function registerRecallTool(ctx: Context): void {
	const tool: ToolDefinition = {
		...
		description:
			"Recover exact evidence and source context behind a compacted observational-memory observation or reflection id. " +
		...
		parameters: {
			...
			properties: {
				id: {
					...
					pattern: "^[a-f0-9]{12}$",
					description:
						"12-character lowercase hex observation or reflection id shown in compacted memory or an om-view listing. " +
		...
		execute: async (args, exec): Promise<RecallToolValue> => {
			const memoryId = typeof (args as { id?: unknown }).id === "string" ? (args as { id: string }).id : "";
