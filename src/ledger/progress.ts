...
export type ConsolidationCoverageType =
...
function isSourceEvent(event: SessionEvent): boolean {
	return event.type === "user/message" || event.type === "assistant/message" || event.type === "tool/result";
}

function coverageSeq(record: LedgerRecord): number | undefined {
...
export function latestCoverageSeq(records: LedgerRecord[], type: ConsolidationCoverageType): number | undefined {
...
export function latestCompaction(records: LedgerRecord[]): CompactionData | undefined {
	for (let i = records.length - 1; i >= 0; i--) {
		const record = records[i];
		if (record?.type === OM_COMPACTION) return record.data;
	}
	return undefined;
}

export function latestFullFoldFirstKeptSeq(records: LedgerRecord[]): number | undefined {
	for (let i = records.length - 1; i >= 0; i--) {
		const record = records[i];
		if (record?.type === OM_COMPACTION && record.data.fullFold) return record.data.firstKeptSeq;
	}
	return undefined;
}
 ...
 */
export function rawTokensSinceSurfaceAnchor(surfaceEvents: readonly SessionEvent[], anchorSeq: number | undefined): number {
	let total = 0;
	for (let i = surfaceEvents.length - 1; i >= 0; i--) {
		const event = surfaceEvents[i];
		if (!event) continue;
		if (anchorSeq !== undefined && event.seq === anchorSeq) break;
		if (event.type === "system/message") continue;
		if (event.type === "user/message") {
			const message = (event.data as { source?: unknown }).source;
			if (message !== undefined && isCompactCheckpointSource(message as Parameters<typeof isCompactCheckpointSource>[0])) {
				break;
			}
		}
		if (isSourceEvent(event)) total += estimateEventTokens(event);
	}
	return total;
}

/** Metered growth since an anchor snapshot; undefined when the basis changed (delta went negative). */
export function realTokensSinceAnchor(currentTokens: number | undefined, anchorTokens: number | undefined): number | undefined {
	if (currentTokens === undefined || anchorTokens === undefined) return undefined;
